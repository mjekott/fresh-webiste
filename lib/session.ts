...
// Constants
export const ACCESS_TOKEN_KEY = "Fresh2Carts-auth-token";
export const REFRESH_TOKEN_KEY = "Fresh2Carts-refresh-token";
...
export const setAuthTokenPair = (
  ...
  accessTokenExpires?: string, // ISO expiry from backend
) => {
  // Access token max age: short if no rememberMe, long if rememberMe
  const accessMaxAge = accessTokenExpires
    ? getMaxAgeFromISO(accessTokenExpires) // use backend value
    : rememberMe
      ? 7 * 24 * 60 * 60 // fallback 7 days
      : 15 * 60; // fallback 15 minutes
  ...
  if (refreshToken && refreshTokenExpires) {
    const refreshMaxAge = rememberMe
      ? 7 * 24 * 60 * 60 // 7 days
      : getMaxAgeFromISO(refreshTokenExpires);
